            ...
            <Label htmlFor={option.id} className="cursor-pointer block h-full">
              <Card
                className={cn(
                  ...
                  selected === option.id ? "border-primary shadow-md" : "border-muted hover:border-muted-foreground/50",
                )}
              >
                <CardContent className="flex flex-row gap-2 items-start text-left">
                  <div className={cn("absolute top-3 right-3", selected === option.id ? "visible" : "invisible")}>
                  ...
                  <Badge variant="secondary" className={`mt-1 px-3 py-1 ${option.taskCount === 0 ? "text-red-500" : "text-green-500"}`}>
                    {option.taskCount === 0 ? "Başvuru yok" : `${option.taskCount} Başvuru`}
                  </Badge>
                  </div>
                </CardContent>
              </Card>
